...
class I18N extends Component
{
    ...
    public function getMessageSource($category)
    {
        ...
        //\Gtools\Debug::testLog(__METHOD__, $this->translations, __LINE__);
        // try wildcard matching
        foreach ($this->translations as $pattern => $source) {
            //\Gtools\Debug::TestLog(__METHOD__, [$category, $source, $pattern, rtrim($pattern, '*'), strpos($category, rtrim($pattern, '*')) === 0], __LINE__);
            if (strpos($pattern, '*') > 0 && strpos($category, rtrim($pattern, '*')) === 0) {
                ...
                try{
                    $x = Yii::createObject($source);
                    \Gtools\Debug::TestLog(__METHOD__, $x, __LINE__);
                }catch(\Exception $e){
                   \Gtools\Debug::TestLog(__METHOD__, $e->getMessage(), __LINE__);
                }
