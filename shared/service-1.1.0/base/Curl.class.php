...
//        \GtoolsDebug::testLog(__FILE__, $content, __METHOD__);
        if (false==$content) {
            $response = array(
                'status' => 'failed',
                'code' => 500,
        ...
        return  ((false===$decode_content) ? $content : $decode_content);
    }
    
    

    public function parseContentToJson($re_content){
        $returned_body = substr($re_content, strrpos($re_content, "\r\n\r\n")+  strlen("\r\n\r\n"));

        return \yii\helpers\Json::decode($returned_body, true);
    }
