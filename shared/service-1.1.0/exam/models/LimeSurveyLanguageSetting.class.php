...
class LimeSurveyLanguageSetting extends ARecord
{
    ...
    public static function getSurveyDetail($id,$user_name)
    {
        if(empty($id)) return [];
        $sql = '';
        foreach ($id as $value){
            $sql .= ' SELECT l.*,t.* FROM '.static::tableName().' l LEFT JOIN lime_tokens_'.$value.' t ON 1=1 WHERE t.firstname="'.$user_name.'" AND l.surveyls_survey_id = '.$value.' UNION';
        }
        $sql = substr($sql,0,-5);
//        return $sql;
