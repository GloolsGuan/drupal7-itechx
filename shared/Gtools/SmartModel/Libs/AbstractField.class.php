<?php
namespace Gtools\SmartModel\Libs;

use \Gtools\SmartModel\Libs\Schema\Field;

abstract class AbstractField extends \yii\base\Component{
    ...
    const RENDER_HTML_ELEMENT = 'element';
    const RENDER_HTML_CODE = 'code';
    const RENDER_HTML_OBJECT = 'node';
...
//        $this->build_group = $field_settings['build_group'];
        ...
        \Gtools\Debug::testLog(__METHOD__, $field_settings, __LINE__);
    ...
    /**
     * Set storage parameters @see \yii\db\ColumnSchema
     * @param type $field_storage
     * @return type
     */
     ...
     */
    public function buildRendering($field_type){
        $this->loadValues();
        $this->beforeBuild();
        $tpl_path = sprintf(dirname(__FILE__) . '/../templates/%s_%s.tpl.php', $field_type, $this->feature);
        ob_start();
        include($tpl_path);
