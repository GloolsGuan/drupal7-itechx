...
define('THEME_LAS_DIR_CSS', THEME_LAS_DIR_BASE.'/css');

require_once(THEME_LAS_DIR_BASE.'/theme-funcs.inc');
...
function lasooo_preprocess_html(&$variables) {
  ...
  _theme_lasooo_load_DefaultCSS($variables);
}
...
function lasooo_process_page(&$variables) {
  ...
  _theme_lasooo_preset_usermenus($variables);
  _theme_lasooo_process_layout($variables);
  _theme_lasooo_load_LayoutCSS($variables);
  _theme_lasooo_load_layoutJS($variables);
}
